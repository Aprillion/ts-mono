...
/**
 * Group turns into agent lanes — the top-level "main" plus each nested subagent
 * — for the focus page's per-subagent turn count and h/l / `<` `>` agent
 * navigation. A turn (model event) belongs to the innermost enclosing
 * `span_begin type="agent"` span, tracked by a depth stack over the flattened
 * tree; arbitrarily deep nesting works since deeper agents are just later lanes.
 * Lanes with no turns are dropped (so "main" disappears when every turn lives in
 * a subagent).
 */
export function computeAgentLanes(flattenedNodes: EventNode[]): AgentLane[] {
  ...
  for (const node of flattenedNodes) {
    ...
    if (event.event === "span_begin" && event.type === "agent") {
      const lane: AgentLane = { name: event.name ?? "agent", turnAnchorIds: [] };
      lanes.push(lane);
