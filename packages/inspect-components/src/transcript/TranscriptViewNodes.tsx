...
export interface TranscriptViewNodesProps {
  ...
  /** Called just before any programmatic scroll (j/k, turn nav, deep-link,
   *  outline/search jumps) so scroll-direction-driven chrome (the swimlane
   *  headroom) can suppress its open/close — the scroll isn't the user
   *  changing direction. Mirrors the URL-`?event=` deep-link suppression that
   *  already exists; passing `true` engages the debounced lock. */
...
// How far (px) the current turn's top may sit above its landed position before
// `k` treats us as "scrolled into the turn" and returns to its top rather than
// stepping to the previous turn. Keeps near-the-top `k` from a pointless nudge.
const kAtTurnTopTolerancePx = 24;
...
>(function TranscriptViewNodes(
  ...
  useEffect(() => {
    ...
    scrollRowToTop(idx);
    // A `?message=` deep link / citation targets a specific message *within*
    // the event — scrolling the event row to the top isn't enough (the message
    // can be far down a long event). After the row mounts, settle the message
    // element just below the sticky chrome, retrying a few frames while
    // Virtuoso / markdown re-measure shifts it.
  ...
  }, [scrollEventId, initialMessageId, flattenedNodes, scrollRowToTop, scrollRef, offsetTop]);
  ...
  // does it step to the previous turn. The current turn's anchor lands at
  // kTranscriptScrollPaddingStart (px from the scroller top); if it sits well
  // above that — or has scrolled out of the DOM above — we're mid-turn. The
  // tolerance keeps "almost at the top" stepping to the previous turn instead of
  // doing a pointless few-px scroll up.
  const onPrev = useCallback(() => {
    ...
    if (container && anchorId) {
      ...
      if (anchorTop < kTranscriptScrollPaddingStart - kAtTurnTopTolerancePx) {
        goToTurn(idx);
        return;
      }
    }
  ...
  // for virtual and non-virtual; at the very bottom the detection point drops so
  // the final turns can still become current). Track *every* row, not just turn
